...
import type { ADOMElement } from './angular-dom';
...
const applyPaddingToText = (node: ADOMElement, text: string): string => {
    ...
    if (yogaNode) {
        ...
        if (node.nodeName === 'ink-root' || node.nodeName === 'ink-box') {
            for (const childNode of node.childNodes) {
                renderNodeToOutput(childNode as ADOMElement, output, {
                    offsetX: x,
                    offsetY: y,
